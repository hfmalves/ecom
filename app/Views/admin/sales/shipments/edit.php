...
<div class="row">
    ...
    <div class="col-lg-4">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title mb-3">Informação da Encomenda</h4>
                <?php $order = $shipment['order'] ?? []; ?>
                <p><strong>#<?= esc($order['id'] ?? '-') ?></strong></p>
                <p>Total: <strong><?= number_format($order['grand_total'] ?? 0, 2, ',', ' ') ?> €</strong></p>
                <p>Estado: <span class="badge bg-primary"><?= esc($order['status'] ?? '-') ?></span></p>
            </div>
        </div>
        ...
        <div class="card mt-4">
            <div class="card-body">
                ...
                <?php $cust = $shipment['customer'] ?? []; ?>
                <p><strong><?= esc($cust['name'] ?? '-') ?></strong></p>
                <p class="text-muted mb-1"><?= esc($cust['email'] ?? '-') ?></p>
                <p class="text-muted mb-0">Grupo: <?= esc($cust['group_name'] ?? '-') ?></p>
            </div>
        </div>
        ...
        <div class="card mt-4">
            <div class="card-body">
                ...
                <?php if (!empty($addr)): ?>
                    <address>
                        <?= esc($addr['street'] ?? '-') ?><br>
                        <?= esc($addr['city'] ?? '-') ?>, <?= esc($addr['postcode'] ?? '-') ?><br>
                        <?= esc($addr['country'] ?? '-') ?>
                    </address>
                <?php else: ?>
        ...
        <div class="card mt-4"
            ...
            <div class="card-body">
                ...
                <!-- Editável apenas se não estiver entregue, devolvido ou cancelado -->
                <template x-if="!['delivered','returned','canceled'].includes(status)">
                    <form @submit.prevent="submit">

                        <div class="mb-3">
                            <label class="form-label">Transportadora</label>
                            <select class="form-select" x-model="form.carrier">
