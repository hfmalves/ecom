    ...
    return {
        ...
        init() {
            this.$root.querySelectorAll("[data-field]").forEach(el => {
                this.form[el.getAttribute("data-field")] = "";
            });
            this.form[config.csrfToken] = config.csrfHash;
        },
        async submitForm() {
            this.loading = true;
            this.errors = {};
            try {
                ...
                // ERROS → CI4 devolve { status: "error", errors: {...} }
                if (data.status === "error" && data.errors) {
                    this.errors = data.errors;
                }

                // SUCESSO → CI4 devolve { status: "success", redirect: "..."}
